<template>
  ...
  <div class="student">
    ...
    <audio ref="praise" :src="audio.jia" style="display:none"></audio>
    <audio ref="criticism" :src="audio.jian" style="display:none"></audio>
    <audio ref="didi" :src="audio.didi" style="display:none"></audio>
    <v-loading v-if="lists.length>0?spinShow=false:spinShow=true"></v-loading>
    <v-header><span>{{class_name}}-{{current_subject.name}}({{lists.length}}名学生)</span></v-header>
    <transition  mode="out-in" enter-active-class="animated fadeInLeft" leave-active-class="animated fadeOutRight">
    <div v-show="!spinShow">
      
   
    <div class="content"  @click.stop="qux"  :style="honer">
    ...
    <div class="biaoshi">
        <ul>
              ...
              <span class="miaoshu">请确认学生考勤状态</span>
    </div>
    <div class="student_lists" :class="icon">
        <div class="student_icon">
            <img :src="apiurl.api+(current_subject.icon?current_subject.icon:'uploads/student/02girl.png')">
        </div>
        <div class="student_name">
            {{ current_subject.name }}
        </div>
    </div>
    <div class="student_lists" :class="icon" v-for="(item,index) in lists" :key="index" @click.stop="add_kaoqin_point(item,index)">


        <div class="attendance">
          <span @click.stop="add_kaoqin(item)"><Icon type="happy" :color="item.attendance_color"></Icon></span>
    ...
    <div class="student_lists" :class="icon" v-for="(item,index) in lists" :key="index" @click="allbehair(item)">
    ...
    <div class="student_lists" :class="icon" v-for="(item,index) in lists" :key="index" @click="addBehair(item)">
    ...
    <div class="student_lists" :class="icon">
        <div class="addstudent" @click="modal3.show=true" v-waves.block>
          <span>+</span>
          <p>添加学生</p>
        </div>
    ...
    <div class="bottom_button" :style="honer">
      ...
      <Button-group shape="circle">
        <Button type="primary" @click="ketang()" v-waves.block>
          课堂
        </Button>
        <Button @click="xiaozu()" v-waves.button> 
          小组
        </Button>
    </Button-group>
      </div>
      ...
      <Dropdown style="margin-left: 20px" trigger="click" placement="top-end">
            <Button type="ghost" shape="circle" v-waves.block>
      ...
      <Poptip placement="top-end" width="450" >
          ...
          <div class="honer_btn" @click="zhankai()" v-waves.block>
                <Icon type="chevron-right" v-show="honer=='right:0px'"></Icon>
                <Icon type="chevron-left" v-show="honer=='right:266px'"></Icon>
          </div>
      </div>
    </div>
    <v-success :modal="success_c" :behair="behair" :item="student_info"></v-success>
    <v-error :modal="error_c" :behair="behair" :item="student_info"></v-error>
    <v-addstudent :modal="modal3" @addstude="addstu"></v-addstudent>
    <v-honer :conlists="lists" v-if="honer_show"></v-honer> 
...
import vHoner from '@/pages/v-honer'
import VSuccess from '@/components/success'
import VError from '@/components/error'
import $ from 'jquery'
...
export default {
  ...
  data () {
    return {
      ...
      xunhuan:'',
      audio:{
        jia:require('@/assets/praise.wav'),
        jian:require('@/assets/criticism.wav'),
        didi:require('@/assets/didi.wav')
      },
      success_c:{
        show:false
      },
  ...
  computed:{
      ...
      behaviorlist:function(){
        return this.$store.getters.get_current_behavior
      }
  },
  watch:{
    lists:{
  ...
  methods:{
    ...
    add_kaoqin_point(item,index){
        this.num=item.attendance_type-1
        this.num++;
      if(this.num==4){
         this.num=0
      }
      console.log(this.num)
      let type=this.attendance[this.num]
      this.xialameun(item,type)
    },
    xialameun(item,type){
    ...
    getUser(){
                    ...
                    success:function(res){
                      if(!res.error_code){
                        ...
                        if(type.list.type==1){
                          ...
                          _this.showscore='+'+type.list.score*1;
                          _this.success_c.show=true
                        }else if(type.list.type==2){
                          _this.$refs.criticism.play()
                          _this.showscore='-'+type.list.score*1;
                          _this.error_c.show=true
                        }
                        
                        _this.$store.commit('add_show_an',{type:type.list})
                        
                        setTimeout(()=>{
                            
                            _this.$store.commit('show_an',{type:false})
                            _this.showscore='';
                            _this.success_c.show=false
                            _this.error_c.show=false
                        },1500)
                      }
                      _this.kill();
                      _this.$store.commit('changecheck',1)
      ...
      getStudent () {
                ...
                $.ajax({
                        ...
                        dataType:'JSON',
                        data:{
                            class_subject_id:_this.class_subject_id,
      ...
      addstu(arr){
          ...
          $.ajax({
                    ...
                    data:{
                        ...
                        user_name:_this.user_info.name,
      ...
      //     let now_time=Date.parse(new Date())/1000+149;
      //     let url=api.api+'pc/report/getrep/format/json'
      //     $.get(url,{time:now_time,class_id:that.class_id,class_subject_id:that.class_subject_id},function(res){
      //         if(res.data){
      //            if(res.data[0].behavior_type==1){
      //                     that.showscore='+'+res.data[0].score;
      //            }else if(res.data[0].behavior_type==2){
      //                     that.showscore='-'+res.data[0].score;
      //           }
      //           that.$store.commit('loop_add',{type:'student',db:res.data})
                
      //             setTimeout(()=>{
      //                  that.$store.commit('show_an',{type:false})
      //                  that.showscore='';
      //             },1500)
      //         }
      //     })
      ...
      // },
      zhankai(){
        this.$store.commit('change_honer')
      }
  ...
  components:{
	 vHeader,vBottom,vLoading,vModel,vRand,vNobottom,vAddstudent,vHoner,VSuccess,VError
  }
