<template>
  <div class="student">
    ...
    <div v-if="grouplist.length>0">
      ...
      <v-header><span>{{class_name}}-{{current_subject.name}}({{grouplist.length}}小组)</span></v-header>
      <transition enter-active-class="animated fadeInRight">
    <div class="content" v-show="!spinShow" :style="honer">
      ...
      <div class="groupList"  @click="joinGroup()" v-waves.button>
        <span class="fuhao">+</span>
        <p class="fuhaofont">添加小组</p>
      </div>
      </div>
      <div class="group" v-if="!group_status.list_show">
    ...
    <div class="bottom_button" :style="honer">
      ...
      <div class="student-tab-btn">
        ...
        <Button @click="ketang()" v-waves.button>
          课堂
        </Button>
        <Button type="primary" @click="xiaozu()" v-waves.button> 
          小组
        </Button>
    </Button-group>
      </div>
      <div class="right_button">
        <div class="honer_btn" @click="zhankai()">
                <Icon type="chevron-right" v-show="honer=='right:0px'"></Icon>
                <Icon type="chevron-left" v-show="honer=='right:266px'"></Icon>
          </div>
      </div>
    </div>
    <v-success :modal="success_c" :behair="behair" :item="group"></v-success>
    <v-error :modal="error_c" :behair="behair" :item="group"></v-error>
    <v-bottom  @suji="rand" @chongzhi="reset" @ping="selecetAll"></v-bottom>
    <v-honer :conlists="grouplist" v-if="honer_show"></v-honer> 
    ...
    <v-addgroup :modal="modal3" @addxz="addxiaozu"></v-addgroup>
    <Modal
        v-model="modal7">
         <p slot="header"> 
            <span>提示信息</span>
...
import vAddgroup from '@/components/addGroup'
import vHoner from '@/pages/v-group-honer'
import VSuccess from '@/components/groupsuccess'
import VError from '@/components/grouperror'
import $ from 'jquery'
function imgUrl(url){
        if (url) {
          let str=url;
...
export default {
    ...
    getUser(){
     ...
     addxw(type){
          ...
          $.ajax({
                    ...
                    success:function(res){
                        ...
                        _this.$store.commit('add_group',{type:type.list})
                        
                        setTimeout(()=>{
                            
                            _this.$store.commit('group_show',{type:false})
                            _this.showscore='';
     ...
     rand(){
      ...
      addxiaozu(item){
        let _this=this;
        $.ajax({
                    url:api.api+'v3/class_group/create/format/json',
                    type:'POST',
                    dataType:'JSON',
                    data:item,
                    success:function(res){
      ...
      //                     that.showscore='-'+res.data[0].score;
      //           }
      //           that.$store.commit('loop_group',{type:'student',db:res.data})
  ...
  components:{
	 vHeader,vBottom,vLoading,vGroup,vGroupmodel,vGrouprand,vAddgroup,vHoner,VSuccess,VError
  }
