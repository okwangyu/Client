...
const mutations = {
  addbehaviorlist(state,action){
    state.current_behavior=action
  },
  ...
  add_show_an(state,action){
    state.student_lists.forEach(function(item){
        if(item.check){
            item.add_show=true
            if(Number(action.type.type)==1){
              item.score=item.score*1+action.type.score*1
              state.group_lists.forEach(function(group){
                group.group_members.forEach(function(vv){
                  if(vv.student_id==item.student_id){
                      vv.score=vv.score*1+action.type.score*1
                  }
                })
              })
            }else if(Number(action.type.type)==2){
              item.score=item.score*1-action.type.score*1
              state.group_lists.forEach(function(group){
                group.group_members.forEach(function(vv){
                  if(vv.student_id==item.student_id){
  ...
  add_group(state,action){
    state.group_lists.forEach(function(item){
        if(item.group_info.check){
            ...
            if(Number(action.type.type)==1){
              item.group_info.score=item.group_info.score*1+action.type.score*1
              item.group_members.forEach(function(vv){
                  vv.score=vv.score*1+action.type.score*1
                  state.student_lists.forEach(function(stu){
                    if(stu.student_id==vv.student_id){
                            stu.score=stu.score*1+action.type.score*1
                    }
                  })
              })
  ...
  loop_add(state,action){
        state.student_lists.forEach(function(item){
            action.db.forEach(function(v){
              if(v.behavior_type==1){
               
                  if(Number(item.student_id)==Number(v.student_id)){
                    item.add_show=true
                    item.score=item.score*1+v.score*1
                    
                    state.group_lists.forEach(function(group){
                      group.group_members.forEach(function(vv){
                        if(Number(vv.student_id)==Number(v.student_id)){
                          vv.score=vv.score*1+v.score*1
                        }
                      })
                    })

                  }
              }else if(v.behavior_type==2){
                  if(Number(item.student_id)==Number(v.student_id)){
                    item.add_show=true
                    item.score=item.score*1-v.score*1
                    state.group_lists.forEach(function(group){
  ...
  loop_group(state,action){
    console.log(action)
    state.group_lists.forEach(function(item){
        action.db.forEach(function(v){
          if(Number(item.group_info.class_group_id)==Number(v.class_group_id)){

              item.group_info.add_show=true
              if(Number(v.behavior_type)==1){
                item.group_info.score=item.group_info.score*1+v.score*1
              }else{
                item.group_info.score=item.group_info.score*1-v.score*1
              }
          }
  ...
  change_honer(state){
    let action=!state.honer_show
    if(action){
        state.honer="right:266px"
        state.honer_show=true
    }else{
        state.honer="right:0px"
        state.honer_show=false
    }
  },
  reset_honer(state){
    state.honer="right:0px"
...
const getters = {
  ...
  get_honer_show(state){
      return state.honer_show
  },
  get_current_behavior(state){
